...
#ifndef ANDROID_WEBVIEW_PUBLIC_BROWSER_DRAW_GL_KITKAT_H_
#define ANDROID_WEBVIEW_PUBLIC_BROWSER_DRAW_GL_KITKAT_H_
...
typedef void (AwDrawGLFunctionKitkat)(int view_context,
                                      AwDrawGLInfoKitkat* draw_info,
                                      void* spare);

// Called to create a GraphicBuffer
typedef int AwCreateGraphicBufferFunctionKitkat(int w, int h);
// Called to release a GraphicBuffer
typedef void AwReleaseGraphicBufferFunctionKitkat(int buffer_id);
// Called to map a GraphicBuffer in |mode|.
typedef int AwMapFunctionKitkat(int buffer_id, AwMapMode mode, void** vaddr);
// Called to unmap a GraphicBuffer
typedef int AwUnmapFunctionKitkat(int buffer_id);
// Called to get a native buffer pointer
typedef void* AwGetNativeBufferFunctionKitkat(int buffer_id);
// Called to get the stride of the buffer
typedef unsigned int AwGetStrideFunctionKitkat(int buffer_id);
// Set of functions used in rendering in hardware mode
struct AwDrawGLFunctionTableKitkat {
  AwCreateGraphicBufferFunctionKitkat* create_graphic_buffer;
  AwReleaseGraphicBufferFunctionKitkat* release_graphic_buffer;
  AwMapFunctionKitkat* map;
  AwUnmapFunctionKitkat* unmap;
  AwGetNativeBufferFunctionKitkat* get_native_buffer;
  AwGetStrideFunctionKitkat* get_stride;
};
...
#endif  // ANDROID_WEBVIEW_PUBLIC_BROWSER_DRAW_GL_KITKAT_H_
